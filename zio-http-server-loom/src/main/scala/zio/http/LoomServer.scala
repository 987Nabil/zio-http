...
class LoomServer(
  ...
  override def serve[Ctx](routes: Routes[Ctx], context: Context[Ctx]): ServerHandle = {
    ...
    // Validate every connector before any socket is bound: H3 is rejected
    // at the model level (ConnectorFailure.H3NotAdvertised) and UDP transport
    // mismatches are rejected at the model level (TcpUdpMismatch), so
    // bindEngines never touches an invalid connector.
